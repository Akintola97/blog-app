...
//           ) : (
//             <p
//               className={`text-gray-600 dark:text-gray-400 ${isDeleted ? "italic text-gray-400" : ""}`}
//             >
//               {comment.content}
//             </p>
//           )}
...
function CommentData({
  ...
  return (
    <div className="mb-6 p-4 capitalize">
      <div className="flex items-start space-x-4">
        <img
          src={comment.user?.picture || `/profile.webp`}
          alt={`${comment.user?.givenName || "User"}'s avatar`}
        ...
        <div className="flex-1">
          <div className="flex justify-between">
            <p className="font-semibold text-lg text-gray-900 dark:text-gray-100 capitalize">
              {comment.user?.givenName}
            </p>
            <div className="flex items-center space-x-2">
              {comment.user.id === comment.userId && (
                <>
                  <button
                    onClick={() => setIsEditing(true)}
                    className={`text-gray-700 dark:text-gray-300 ${isDeleted ? "cursor-not-allowed opacity-50" : "hover:text-blue-600"}`}
                    aria-label="Edit"
                    disabled={isDeleted}
                  >
                    <FiEdit />
                  ...
                  <button
                    onClick={handleDeleteClick}
                    className={`text-gray-700 dark:text-gray-300 ${isDeleted ? "cursor-not-allowed opacity-50" : "hover:text-red-600"}`}
                    aria-label="Delete"
                    disabled={isDeleted}
                  >
                    <FiTrash2 />
          ...
          <div className="mt-2">
            {isEditing ? (
              ...
              <p
                className={`text-gray-800 dark:text-gray-200 ${isDeleted ? "italic text-gray-400" : ""}`}
              >
                {comment.content}
              </p>
            )}
          ...
          {showReplies && comment.replies?.length > 0 && (
            <div className="mt-4 bg-black bg-opacity-10 text-gray-900 dark:bg-gray-800 dark:text-gray-200 p-4 rounded-lg">
              {comment.replies.map((reply) => (
                <div key={reply.id} className="py-2">
                  <div className="font-semibold">{reply.user?.givenName}</div>
